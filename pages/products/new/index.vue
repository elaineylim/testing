<template>
  <div class="flex flex-col">
    ...
    <contentCard class="mt-2 mb-4">
      <template v-slot:title>New Bean</template>
      <template v-slot:content>
...
export default {
  ...
  methods: {
    ...
    waterLose() {
      let wL =
        this.weightAfterRoast && this.weightBeforeRoast
          ? (1 - this.weightAfterRoast / this.weightBeforeRoast) * 100
          : 0.0;
      return wL;
    },
    stockPerKg() {
    ...
    roastedBeanPerKg() {
      let rBKG =
        Object.keys(this.selectedStock).length > 0 && this.weightAfterRoast
